...
//activity log
    ...
    public function inactiveAseListASM(Request $request)
    {
        ...
        // 🔹 Handle "Both"
        $brandsToCheck = ($brandCode == 3) ? [1, 2] : [$brandCode];
        $aseDetails = Team::select('employees.id')->join('employees', 'teams.ase_id', '=', 'employees.id')->where('teams.asm_id', '=', $userId)->where('brand','=',$brandCode)->where('teams.status',1)->where('teams.is_deleted',0)->groupby('teams.ase_id')->orderby('teams.ase_id')->get()->pluck('employees.id')->toArray();
        ...
        $inactiveASE=Team::select('employees.*')->join('employees', 'teams.ase_id', '=', 'employees.id')->where('teams.asm_id', '=', $userId)->whereNotIn('users.id',$activeASEreport)->where('teams.asm_id', '=', $userId)->where('brand','=',$brandCode)->where('teams.status',1)->where('teams.is_deleted',0)->groupby('teams.ase_id')->orderby('teams.ase_id')->get();
