    ...
    public function index(Request $request)
{
    ...
    if ($request->filled('brand_selection')) {
        ...
        if ($selected == 3) {
            $query->whereIn('retailer_barcodes.brand', [1, 2]);
        } else {
            // single brand selected → include that + both
                    $query->where('retailer_barcodes.brand', $request->brand_selection)
    ...
    public function qrRedeem(Request $request)
    {
        ...
        $query->when($keyword, function ($q) use ($keyword) {
            $q->where(function ($k) use ($keyword) {
                ...
                $k->where('stores.store_name', 'LIKE', "%$keyword%")
                    ...
                    ->orWhere('stores.area', 'LIKE', "%$keyword%")
                    ->orWhere('stores.state', 'LIKE', "%$keyword%")
                    ->orWhere('stores.pin', 'LIKE', "%$keyword%")
                    ->orWhere('stores.contact_person', 'LIKE', "%$keyword%")
    ...
    public function sequenceSave(Request $request)
    {
        ...
        $request->validate([
            ...
            // Check if any existing range overlaps
            $overlap = QrSequence::where(function ($query) use ($newFrom, $newTo) {
                $query->whereBetween('from', [$newFrom, $newTo])
                      ->orWhereBetween('to', [$newFrom, $newTo])
                      ->orWhereRaw('? BETWEEN `from` AND `to`', [$newFrom])
                      ->orWhereRaw('? BETWEEN `from` AND `to`', [$newTo]);
            })->exists();

            if (!$overlap) {
       
                // $qr=QrSequence::where('from',$request['from'])->where('to',$request['to'])->first();
                // if(empty($qr)){
                    $storeData = new QrSequence;
                    $storeData->distributor_id = $request['distributor_id'];
                    $storeData->from = $request['from'];
            	    $storeData->to = $request['to'];
                    $storeData->count = $request['count'];
        		    $storeData->actual_date = $request['actual_date'];
                    $storeData->save();
                    if ($storeData) {
                    return redirect()->back()->with('success', 'Submitted');
                    } else {
                        return redirect()->back()->withInput($request->all())->with('success', 'Something happened');
                    }
            }else{
                     return redirect()->back()->withInput($request->all())->with('failure', 'Already assigned to distributor');
            }
            
        
    }

    public function sequenceCsv(Request $request)
    {
        
        $qr = QrSequence::orderby('created_at')->get();

        // Check if there is data to export
        if (count($qr) > 0) {
                $delimiter = ",";
                $filename = "QR code sequence report-".date('Y-m-d').".csv";
                ...
                $fields = array('SR','DISTRIBUTOR', 'SEQUENCE FROM', 'SEQUENCE TO','COUPON COUNT','ACTUAL DATE', 'DATE');
                fputcsv($f, $fields, $delimiter);
                ...
                foreach($qr as $row) {
                    ...
                    $lineData = array(
                        ...
                        $row->to,
                        $row->count,
                        date('j F, Y', strtotime($row->actual_date)),
                        date('j F, Y h:i A', strtotime($row->created_at))
    ...
    // public function errorlogCsv(Request $request)
    // {
    //     $qr = DB::table('failed_logs')
    //             ->orderBy('created_at')
    //             ->get();
    ...
    //     $filename = "Error_log_report_" . date('Y-m-d') . ".csv";
    ...
    public function couponmismatchCsv(Request $request)
    {
        // Clear any previous output that breaks CSV downloads
        if (ob_get_level()) {
            ob_end_clean();
        }
        ...
        $filename = "Serial_Number_Mismatch_Coupon_Report_" . date('Y-m-d') . ".csv";
        ...
        return Response::streamDownload(function () use ($from, $to) {
            ...
            fputcsv($handle, [
                'SR', 'QRCODE TITLE', 'CODE', 'SERIAL NUMBER',
                'DISTRIBUTOR', 'ASE', 'STORE UNIQUE CODE',
                'STORE NAME', 'STORE MOBILE', 'STORE EMAIL',
                'STORE STATE', 'STORE ADDRESS', 'ASSIGN DISTRIBUTOR',
                'ASSIGN DISTRIBUTOR STATE', 'ASSIGN DISTRIBUTOR CITY',
                'POINTS', 'DATE'
            ]);
            ...
            ->chunk(500, function ($rows) use (&$count, $handle) {
                ...
                foreach ($rows as $row) {
                    fputcsv($handle, [
                        ...
                        $row->email,
                        $row->state,
                        $row->address . ' ,' . $row->area . ' ,' . $row->pin,
                        $row->assign_distributor,
                        $row->assign_distributor_state,
                        $row->assign_distributor_city,
                        $row->amount,
                        date('j F, Y h:i A', strtotime($row->created_at))
                    ]);
                }
